...
SELECT EMP_NAME AS "이름", SALARY*12 AS "연봉", (SALARY*12+SALARY*NVL(BONUS, 0)) AS "총수령액", 
(SALARY*12+SALARY*NVL(BONUS,0))-(SALARY*0.03) AS "실수령액" FROM EMPLOYEE;
-- 2. EMPLOYEE 테이블에서 이름, 근무 일수를 출력해보시오.
-- (SYSDATE를 사용하면 현재 시간 출력)
SELECT EMP_NAME AS "이름", ROUND(SYSDATE - HIRE_DATE) AS "근무일수" FROM EMPLOYEE;
...
ROUND(ENT_DATE - HIRE_DATE) "근무기간", ENT_DATE "퇴직일" FROM EMPLOYEE WHERE ENT_YN = 'Y';
-- 문제3.
...
WHERE (ROUND(SYSDATE - HIRE_DATE) >= 3650) ORDER BY 3 ASC; -- 3번째 컬럼을 기준으로 오름차순하겠다
-- 문제4.
...
WHERE EMP_NO LIKE '__04__-2%' AND SALARY BETWEEN 2000000 AND 3000000
...
SELECT EMP_NAME "이름", FLOOR(FLOOR(SYSDATE - HIRE_DATE) / 1000)*(SALARY*0.1) "특별 보너스" FROM EMPLOYEE
WHERE (EMP_NO LIKE'%-1%') AND BONUS IS NULL ORDER BY EMP_NAME ASC;
