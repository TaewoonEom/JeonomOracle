...
--	ex) 노옹철	no_hc
--	ex) 정중하	jung_jh
-- EMAIL의 처음부터 @위치-1번째 위치까지 자른다.
SELECT EMP_NAME "이름",  SUBSTR(EMAIL, 1, INSTR(EMAIL, '@')-1) "이메일 아이디",
INSTR(EMAIL, '@') "@위치" FROM EMPLOYEE;
...
RPAD(SUBSTR(EMP_NO, 1, 8), 14, '*')FROM EMPLOYEE;   

--7. 부서코드가 D5, D9인 직원들 중에서 2004년도에 입사한 직원중에 조회함.
--   사번 사원명 부서코드 입사일
SELECT EMP_ID "사번", EMP_NAME "사원명", DEPT_CODE "부서코드", HIRE_DATE "입사일"
FROM EMPLOYEE WHERE DEPT_CODE IN('D5', 'D9') AND EXTRACT(YEAR FROM HIRE_DATE) = '2004';

--8. 직원명, 입사일, 오늘까지의 근무일수 조회 
--	* 주말도 포함 , 소수점 아래는 버림
SELECT EMP_NAME "직원명", HIRE_DATE "입사일", FLOOR(SYSDATE-HIRE_DATE)+1 "근무일수" FROM EMPLOYEE;

--9. 직원명, 부서코드, 생년월일, 나이(만) 조회
--   단, 생년월일은 주민번호에서 추출해서, 
--   ㅇㅇㅇㅇ년 ㅇㅇ월 ㅇㅇ일로 출력되게 함.
--   나이는 주민번호에서 추출해서 날짜데이터로 변환한 다음, 계산함
SELECT EMP_NAME "직원명", DEPT_CODE "부서코드", '19'||SUBSTR(EMP_NO, 1, 2)||'년 '
...
-- 자동 형변환 안되는 경우
SELECT '1,000,000' - '500,000' FROM DUAL; -- 01722. 00000 -  "invalid number"
