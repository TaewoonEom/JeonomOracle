...
WHERE DEPT_TITLE LIKE '해외%' AND BONUS IS NOT NULL;
...
-- 데이터 없음!
SELECT EMP_NAME, JOB_NAME, DEPT_TITLE, SALARY, SALARY*12 FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
...
SELECT DECODE(ENT_YN, 'Y', '재직', 'N', '퇴사', '무') "퇴직 여부", COUNT(*) "인원수" FROM EMPLOYEE
GROUP BY ENT_YN;