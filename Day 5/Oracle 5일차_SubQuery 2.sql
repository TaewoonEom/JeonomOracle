...
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
JOIN LOCATION ON LOCATION_ID = LOCAL_CODE
WHERE LOCAL_NAME = (SELECT LOCAL_NAME FROM LOCATION WHERE LOCAL_NAME = 'ASIA1');
-- 서브쿼리 쓰는 경우 2
SELECT DEPT_CODE, EMP_NAME FROM EMPLOYEE
WHERE DEPT_CODE IN (
SELECT DEPT_ID FROM DEPARTMENT JOIN LOCATION ON LOCATION_ID = LOCAL_CODE 
WHERE LOCAL_NAME = 'ASIA1');
-- 서브쿼리 쓰는 경우 3 -- 서브쿼리 안에 서브쿼리 쓰는 경우
SELECT DEPT_CODE, EMP_NAME FROM EMPLOYEE
WHERE DEPT_CODE IN (
...
SELECT * FROM EMPLOYEE WHERE EMP_ID = '2000'; -- RESULT SET 없음

SELECT * FROM EMPLOYEE WHERE 1=0; -- RESULT SET 없음
SELECT * FROM EMPLOYEE WHERE 1=1; -- RESULT SET 있음

SELECT * FROM EMPLOYEE WHERE EXISTS (SELECT 1 FROM EMPLOYEE); -- RESULT SET 있음

SELECT * FROM EMPLOYEE E WHERE EXISTS (SELECT 1 FROM EMPLOYEE WHERE MANAGER_ID = E.EMP_ID); -- RESULT SET 없음
...
SELECT 1 FROM EMPLOYEE WHERE MANAGER_ID = '201'; -- 0
...
SELECT * FROM EMPLOYEE WHERE EMP_ID IN ('200', '201', '204', '207');
...
SELECT * FROM EMPLOYEE E WHERE NOT EXISTS (SELECT 1 FROM EMPLOYEE WHERE E.SALARY < SALARY);
...
GROUP BY DEPT_CODE
HAVING DEPT_CODE = 'D5'; -- D5 부서의 월평균급여, 2번

SELECT DEPT_CODE, EMP_NAME, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE
GROUP BY DEPT_CODE HAVING DEPT_CODE = 'D5')  "월평균급여" FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'; -- 메인쿼리에 SELECT 뒤에 월평균급여 서브쿼리 사용, 3번
