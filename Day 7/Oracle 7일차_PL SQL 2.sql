...
BEGIN
    ...
    WHERE EMP_ID = '&EMP_ID';
    -- 결과 출력
    DBMS_OUTPUT.PUT_LINE('사원번호 : ' || VEMPID || ', 이름 : ' || VENAME
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, PONE, JOB_CODE, SAL_LEVEL)
VALUES(LAST_NUM+1, '&NAME', '&EMP_NO','&PHONE', '&JOB_CODE', '&SAL_LEVEL');
...
BEGIN
    ...
    INTO EMPID, ENAME, DTITLE, VJNAME, VJCODE
    FROM EMPLOYEE
    LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    LEFT OUTER JOIN JOB USING(JOB_CODE)
    WHERE EMP_ID = '&EMP_ID';
    ...
    ELSE DBMS_OUTPUT.PUT_LINE('소속 : 일반 직원');
    END IF;
END;
/
