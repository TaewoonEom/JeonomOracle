...
SELECT E.* FROM     -- ROWNUM 생략해도 똑같은 결과 출력됨
...
-- 기술할 수 있음. 같은 서브쿼리가 여러번 사용될 겨우 중복 작성을
-- 피할 수 있고 실행속도도 빨라지는 장점이 있음.
-- 사용방법
-- WITH 서브쿼리명 AS (서브쿼리)
-- SELECT * FROM (서브쿼리명)
-- 급여 TOP 5인 직원의 전체정보를 출력하시오
WITH TOP5_SAL AS
(SELECT * 
FROM EMPLOYEE 
